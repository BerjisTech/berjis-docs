...
func New(opts Options) *fiber.App {
	...
	if coreAPIBase != "" {
		if v, err := coreauth.NewVerifier(coreauth.Config{
			...
			fmt.Printf("warn: docs coreauth verifier init failed: %v\n", err)
		} else {
			authVerifier = v
		}
	}
		...
		if token != "" && authVerifier != nil {
			if claims, err := authVerifier.Verify(token); err == nil {
				...
				if !errors.Is(err, coreauth.ErrJWKSUnavailable) {
					fmt.Printf("warn: docs coreauth verify failed: %v\n", err)
				}
			}
		}
	...
	// Status changes
	app.Post("/v1/docs/:id/archive", func(c *fiber.Ctx) error { return setStatus(c, "archived") })
	app.Post("/v1/docs/:id/restore", func(c *fiber.Ctx) error { return setStatus(c, "active") })
	app.Delete("/v1/docs/:id", func(c *fiber.Ctx) error { return setStatus(c, "deleted") })
